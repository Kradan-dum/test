...
$_SESSION["folder"] = $_GET['idr'];
$folder = $_GET['idr'];
// echo $folder;

$_SESSION["RF"]["subfolder"] = $_GET['idr'];
...
$config = array(
	...
	*/
	...
	'base_url' => ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && ! in_array(strtolower($_SERVER['HTTPS']), array( 'off', 'no' ))) ? 'https' : 'http') . '://' . $domain_name,
	...
	*******************/
	...
	'fixed_path_from_filemanager'             => array( '../1', '../test1/' ), //fixed path of the image folder from the current position on upload folder
